...
/** RELOCATE_MARKER as a "%s %s %s:%s" splice.  EVERY emitter -- the
 * sendcmdto_flag_serv_butone() broadcast and X3's own emission alike --
 ...
 * peers advertise the 'r' SERVER flag (FLAG_RENAME_CAPABLE); services
 * are exempt: a services package that cares about RENAME advertises 'r'
 * itself and then receives RN like any other capable peer, while one
 * that does not is simply left out of the broadcast.
...
static struct Client *rename_legacy_blocker(void)
{
  ...
  for (acptr = GlobalClientList; acptr; acptr = cli_next(acptr))
    if (IsServer(acptr) && !IsRenameCapable(acptr) && !IsService(acptr))
      return acptr;
...
void pending_rename_complete(struct PendingRename *pr)
{
  ...
  /* Re-check for a legacy blocker: a non-rename-capable, non-service server
   * can link during the up-to-RENAME_TIMEOUT-second AC round-trip, after
   ...
   * Runs before the consent branch — a relocation onto a banned name is
   * just as much a bypass as a classic rename onto one. */
  {
  ...
  if (rename_is_consent()) {
    ...
    /* Propagate to rename-capable ('r') servers; the legacy-blocker guard
     * (in m_rename) guarantees every non-service peer has the flag. */
    sendcmdto_flag_serv_butone(pr->client, CMD_RENAME, pr->client,
                               FLAG_RENAME_CAPABLE, FLAG_LAST_FLAG,
  ...
  /* Propagate to rename-capable ('r') servers; a peer that never
   * advertised the flag would neither apply nor relay RN, and the
   * legacy-blocker guard (in m_rename) guarantees no such peer (services
   * excepted) is linked. */
  sendcmdto_flag_serv_butone(pr->client, CMD_RENAME, pr->client,
                             FLAG_RENAME_CAPABLE, FLAG_LAST_FLAG,
...
static void relocate_tombstone_sweep(struct RelocateTombstone *ts)
{
   ...
   * next_member cursor: remove_user_from_channel() frees the member node,
   * so even a next_member captured before the call can be freed by it (the
   * hazard relocate_execute() solves with a snapshot array -- here the
   * list is being emptied outright, so re-reading the head is simpler and
   * needs no allocation).  Progress is guaranteed because
   * find_member_link() -- unlike find_channel_member() (channel.c:496) --
   * returns zombie memberships too, so the selected member is always
   * actually removed. */
  while ((member = chptr->members)) {
     ...
     * exclude the source (one == NULL), so a local member sees the PART. */
  ...
  /* else: any member still linked here keeps chptr->members > 0, so
   * sub1_from_channel() would refuse to collect anyway.  Assert nothing:
   * the marks are cleared, so the channel is now an ordinary channel and
   * the ordinary path collects it when the last member goes. */
                            ...
                            const char *reason)
{
   ...
   * exceed chptr->users: zombies are on the list too).  Do it
   ...
   * remove_user_from_channel() in pass two frees the moving member's node
   * from this same list, so a next_member cursor walking it while removing
   * could be left dangling.  Snapshotting the movers first makes that
...
int m_rename(struct Client *cptr, struct Client *sptr, int parc, char *parv[])
{
   ...
   * up front while a legacy (non-rename-capable, non-service) server is on
   * the network, rather than only in the registered-channel branch below —
  ...
  if (rename_is_consent()) {
    ...
    /* Propagate to rename-capable ('r') servers; the legacy-blocker guard
     * above guarantees every non-service peer has the flag. */
    sendcmdto_flag_serv_butone(sptr, CMD_RENAME, cptr,
                               FLAG_RENAME_CAPABLE, FLAG_LAST_FLAG,
                               "%s %s %s:%s", oldname_buf, newname,
  ...
  sendcmdto_flag_serv_butone(sptr, CMD_RENAME, cptr,
                             FLAG_RENAME_CAPABLE, FLAG_LAST_FLAG,
                             "%s %s :%s", oldname_buf, chptr->chname, reason);
...
int ms_rename(struct Client *cptr, struct Client *sptr, int parc, char *parv[])
{
  ...
  if (relocate) {
    ...
    /* Propagate with the marker preserved, to rename-capable ('r') servers,
     * skipping the link this RENAME arrived on. */
    sendcmdto_flag_serv_butone(sptr, CMD_RENAME, cptr,
                               FLAG_RENAME_CAPABLE, FLAG_LAST_FLAG,
                               "%s %s %s:%s", oldname_buf, newname,
                               RELOCATE_MARKER_SP, reason);
  ...
  /* Propagate to rename-capable ('r') servers, skipping the link this
   * RENAME arrived on. */
  sendcmdto_flag_serv_butone(sptr, CMD_RENAME, cptr,
                             FLAG_RENAME_CAPABLE, FLAG_LAST_FLAG,
                             "%s %s :%s", oldname_buf, chptr->chname, reason);
