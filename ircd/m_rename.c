 ...
 * node when the new name is longer than the old one — channel.c) or
 * destroyed outright (last member parts mid-flight) while this request
...
void pending_rename_complete(struct PendingRename *pr)
{
  ...
  /* Re-check for a legacy blocker: a non-rename-capable, non-service
   * server can link during the up-to-RENAME_TIMEOUT-second AC
   * round-trip, after m_rename's request-time check already passed.
   * Nothing has been applied anywhere on the network yet at this point
   * (the rename below is the first mutation), so deny rather than
   * complete — that's the conservative move that keeps every server
   * consistent. */
   ...
   * legacy-blocker guard above guarantees no such peer (services
   * excepted) is linked. */
...
int m_rename(struct Client *cptr, struct Client *sptr, int parc, char *parv[])
{
   ...
   * up front while a legacy (non-rename-capable, non-service) server is
   * on the network, rather than only in the registered-channel branch
   * below — an unregistered-channel rename that a legacy peer can
   * neither relay nor apply diverges it from the rest of the network
   * just as surely. */
  {
